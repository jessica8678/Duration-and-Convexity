...
## What this is
A single-file, dependency-free educational HTML page (pricing → curve roles → delta → gamma for a EUR IRS): `swap-duration-convexity.html`. No build step, no backend. It must always still open standalone in a browser after any edit. This memory file governs work on that page.

The repo also ships a second, separate product: `index.html`, the bond Duration & Convexity Lab (v1.0, live on GitHub Pages, documented in README.md). `duration-convexity.html` + `duration-convexity-study.html` are its predecessors (SPEC.md/STUDY.md and `test/` describe them). The swap lab is not an "older version" of index.html — don't archive it as one. The repo is public; every push goes live via Pages.
...
6. **Edits only count on the real file, via Claude Code in this project folder.** A session without direct disk access to this project (a chat sandbox, another machine) can't run `/verify` or make edits that count as done — say so explicitly, and hand off the exact instruction to run in the session that does have access.